...
/* ----------------------------- banco de dados (Upstash Redis, persistente de verdade) -----------------------------
   O Render free apaga o disco local a cada hibernação/redeploy — por isso NÃO usamos mais
   um arquivo .json no servidor. O Upstash é um banco de chave-valor gratuito que guarda os
   dados de verdade, fora do servidor, então eles nunca se perdem sozinhos.
   Guardamos o banco inteiro como um único JSON dentro da chave "licencas_db". Pro tamanho
   de clientes de um app como esse, isso é simples e mais que suficiente. */
...
async function lerDB() {
    if (!UPSTASH_URL || !UPSTASH_TOKEN) return { licencas: {} }; // sem Upstash configurado, roda "no vácuo" (não persiste)
    try {
...
async function obterOuCriarLicenca(deviceId, nome, telefone, ip) {
    ...
    if (!db.licencas[deviceId]) {
        ...
        // Toda checagem (o app abrindo, reverificando) atualiza quando foi "visto" pela última vez —
        // é o que permite saber quem realmente ainda usa o app.
...
// Pega o IP real do cliente mesmo atrás do proxy do Render.
...
// Também sinaliza (sem bloquear sozinho) quando o mesmo IP já criou outra licença —
// indício de reinstalação pra "resetar" o teste grátis, pra você revisar e decidir.
app.get('/api/admin/licencas', exigirAdmin, async (req, res) => {
    ...
    // Prioridade pra ordenação: pago > trial > expirado — assim quem paga não fica
    // enterrado embaixo de gente que só testou uma vez e nunca mais voltou.
    ...
    const lista = todas
        ...
        .sort((a, b) => {
            ...
            if (p !== 0) return p;
            return a.dias_inativo - b.dias_inativo; // dentro do mesmo grupo, quem acessou mais recente primeiro
        });
    const resumo = {
        total: lista.length,
...
// Apaga um cliente/licença específico definitivamente (ex.: alguém que só testou e nunca voltou).
...
// Limpeza em massa: apaga quem NUNCA pagou e não acessa o app há mais de X dias.
// Nunca mexe em quem já pagou alguma vez, mesmo que esteja inativo — só limpa teste "morto".
app.post('/api/admin/licencas/limpar-inativos', exigirAdmin, async (req, res) => {
    ...
    for (const id of Object.keys(db.licencas)) {
        ...
        if (l.pago) continue; // nunca apaga quem já pagou alguma vez
        const ultimaVez = l.ultima_verificacao || l.criado_em;
        const diasInativo = Math.floor((agora - new Date(ultimaVez).getTime()) / MS_DIA);
